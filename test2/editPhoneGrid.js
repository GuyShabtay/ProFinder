...
describe("edit Phone test", function () {
    it("edit Phone test on Chrome", async function () {
    ...
    it("edit Phone test on Edge", async function () {
        let driver = await new Builder()
            .usingServer(GRID_URL)
...
async function runTest(driver) {
    try {
        ...
        const secondInputField = inputFields[2];

        await secondInputField.clear();
        await secondInputField.sendKeys('22222');
