...
describe("edit profession test", function () {
    it("edit profession test on Chrome", async function () {
    ...
    it("edit profession test on Edge", async function () {
        let driver = await new Builder()
            .usingServer(GRID_URL)
...
async function runTest(driver) {
    try {
        ...
        const secondInputField = inputFields[0];

        await secondInputField.clear();
        await secondInputField.sendKeys('sales man');
